...
ID = 318735156  # TODO: change it to your personal ID
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    # dist(x1[i], x2[j]) = sqrt(||x1[i]||^2 + ||x2[j]||^2 - 2 * x1[i] dot x2[j])
    dists = np.sqrt(x1_squared_norms + x2_squared_norms - 2 * np.dot(x1, x2.T))
    # ========================
